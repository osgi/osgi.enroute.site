...
%%% My (own custom) Management Agent (calling Configuration Admin direct) does not seem to be able to configure the component. Is there something special that needs to be done for the Configuration to take effect? Note: the component does not require a configuration, but the configuration tries to override the properties. My expectation is that the `@Modified` method should get called, but nothing happens.
